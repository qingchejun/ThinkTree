...
export default function OutlineMindmap({ markdown, mindmapId, editable = false, onSaved, meta }) {
        ...
        <div className={`article-header px-4 py-2 text-[16px] font-semibold text-[#2c3e50] border-l-4 border-[#3498db] hover:bg-[#f8f9fa] flex items-center ${editable ? 'hover:border hover:border-dashed hover:border-purple-300 rounded-sm' : ''}`}>
          ...
          {editable && editing.id === node.id ? (
            <input
              ...
              onChange={(e) => setEditing({ id: node.id, text: e.target.value })}
              onKeyDown={(e) => {
                if (e.key === 'Enter') { e.stopPropagation(); commitEdit(node.id) }
  ...
  const cancelEdit = () => setEditing({ id: null, text: '' })
  const commitEdit = (id) => {
    if (!tree) return
    const update = (n) => {
      if (n.id === id) {
        n.label = stripHtmlTags(editing.text || '').trim()
        return true
      }
      let changed = false
      n.children?.forEach((c) => { if (update(c)) changed = true })
      return changed
    }
  ...
  // 树转 Markdown（与构建逻辑对偶）
  const treeToMarkdown = (root) => {
    const lines = []
    const visit = (n) => {
      if (n.level === 0) {
        lines.push(`# ${stripHtmlTags(n.label || '思维导图')}`)
      } else if (n.level === 1) {
        lines.push(`## ${stripHtmlTags(n.label || '')}`)
      } else if (n.level === 2) {
        lines.push(`### ${stripHtmlTags(n.label || '')}`)
      } else {
        lines.push(`${'  '.repeat(Math.max(0, n.level - 3))}- ${stripHtmlTags(n.label || '')}`)
      }
      n.children?.forEach(visit)
    }
    visit(root)
