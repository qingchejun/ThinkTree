...
Feature: 积分系统 (ThinkTree Credits)
Status: 规划中
Owner: @thinkso-team
Last Updated: 2025-07-28

🎯 1. 目标与价值 (Objective & Value)
对用户 (For Users): 建立一个清晰、透明、可预测的服务用量体系。用户可以明确知道每次操作的成本，并通过多种方式获取免费使用额度，从而更好地规划和使用产品。

对产品 (For Business):
驱动用户增长: 在产品发布初期，通过慷慨的积分赠送策略，低成本、大规模地获取早期核心用户，快速验证产品价值。
成本控制与价值量化: 将服务使用量与 AI API 成本挂钩，有效控制运营成本，并精准地衡量和体现产品功能的核心价值。
运营灵活性: 赋予管理员账号特殊权限，并提供积分兑换码工具，以支持市场活动、用户补偿等多种运营场景。
商业化基础: 为未来的订阅制、按量付费等商业化模式搭建必要的基础设施。

✅ 2. 验收标准 (Definition of Done - V1 Launch)
此标准定义了第一版（MVP）上线的最低要求，确保免费用户获得一个完整、流畅且富有吸引力的核心体验。
[ ] 上线推广期注册即赠送: 为实现快速用户增长，在上线推广期内注册的新用户，账户中自动获得 1000 初始积分。
[ ] 余额随时可见: 用户登录后，在页面顶部导航栏能清晰看到自己的积分余额。
[ ] 操作前有预估: 在用户输入文本或上传文件后，生成按钮旁会实时显示本次操作的预计消耗积分。
[ ] 积分正确扣除: 用户成功生成思维导图后，系统会从其账户中扣除相应的积分，导航栏余额实时更新。
[ ] 积分不足有提醒: 当预估积分大于用户余額时，生成操作被阻止，并弹出友好提示，引导用户了解积分获取途径。
[ ] 消费历史可追溯: 用户可以在个人中心查看自己最近的积分增减记录（何时、何事、多少分）。
[ ] 管理员权限生效: 使用管理员账号进行操作时，不消耗任何积分。

📋 3. 任务分解 (Task Breakdown)
Phase 1: 核心功能 MVP (Core MVP for V1 Launch)
此阶段任务为 V1 上线的最高优先级
...
在 User 模型中添加 credits (Integer) 和 role (String, e.g., 'user', 'admin') 字段。
创建 CreditHistory 数据模型，字段包括: user_id, change_amount, balance_after, reason, related_id, created_at。

[ ] 核心逻辑:

管理员权限: 在积分检查逻辑中，始终对 role 为 'admin' 的用户放行，不检查也不扣除其积分。
为新注册用户设置可配置的初始积分。（上线推广期，此配置需设置为 1000）。
在调用 AI 核心服务前，实现对普通用户的原子性积分检查与扣除机制（使用数据库事务 SELECT ... FOR UPDATE）。
积分成功扣除后，在 CreditHistory 表中创建一条消费记录。

[ ] API:

创建 GET /api/credits/history 接口，支持分页，返回当前用户的积分明细列表。
前端 / UI (Frontend / UI)
[ ] 状态管理: 在全局用户状态 (如 AuthContext) 中添加 credits 字段，并实现更新函数。

[ ] UI 组件:
在 Header 组件中，显示用户剩余积分。
在生成页面，根据用户输入内容（文本长度等），调用 creditUtils.js 实时计算并显示预计消耗。
创建“积分明细”页面，调用 GET /api/credits/history 接口并以列表形式展示数据。
完善 CreditAlert.jsx 组件，当积分不足时，提供明确的提示信息。

测试 (Testing)
[ ] 单元测试: 针对积分扣除的 service 层逻辑编写单元测试。
[ ] 集成测试: 覆盖从前端点击生成 -> 后端扣分 -> AI 服务调用 -> 返回结果的完整流程。
[ ] 并发测试: 模拟多个请求同时扣除积分，验证事务锁是否生效，确保数据一致性。
[ ] 权限测试: 验证管理员账号操作不扣分，普通用户账号正常扣分。

Phase 2: 免费用户生态与运营工具 (Post-Launch Iteration)
此阶段任务在 V1 上线后，作为高优先级迭代

后端 / API (Backend / API)

[ ] 常规积分策略: 推广期结束后，将新用户注册赠送积分的默认配置调整为 100。

[ ] 邀请功能 (用户增长):
在 User 模型添加 invitation_code 和 invited_by 字段。
开发邀请逻辑：当新用户使用邀请码注册后，为邀请人增加指定积分。

[ ] 每日奖励:
开发一个每日任务或签到逻辑，用户每日首次登录或主动签到时，可领取少量积分（例如：20 积分）。

[ ] 积分兑换码 (运营工具):

数据库: 创建 CreditRedemptionCode 数据模型，字段包括: code (unique), credits_amount, is_active, expires_at, created_by_admin_id, used_by_user_id, used_at。

管理后台 API:
POST /api/admin/redemption-codes: 仅限管理员调用，用于生成指定额度的积分兑换码。

用户端 API:
POST /api/credits/redeem: 用户提交兑换码，验证其有效性后，为用户增加相应积分，并将兑换码状态置为已使用。

前端 / UI (Frontend / UI)
[ ] 邀请页面: 创建一个“邀请好友”页面，显示用户的专属邀请码/链接，并说明奖励规则。
[ ] 签到组件: 在首页或用户中心添加一个醒目的签到入口。
[ ] 兑换码输入: 在用户中心的“积分”或“账户”页面，增加一个输入框和按钮，用于提交积分兑换码。
[ ] 管理后台页面 (Admin Panel):
创建一个简单的内部管理页面，允许管理员生成和查看积分兑换码列表及其状态。

💡 4. 注意事项与技术决策 (Notes & Technical Decisions)
技术决策 (Technical Decisions)

权限分离: 系统的权限控制必须清晰。管理员 (Admin) 角色拥有无限积分和生成兑换码的权限。普通用户 (User) 的积分受系统规则限制，只能通过注册、邀请、签到、兑换等方式获取。
邀请码 vs 兑换码:
用户邀请码 (invitation_code): 用于追踪用户间的邀请关系，实现病毒式增长，与新用户注册流程绑定。
积分兑换码 (redemption_code): 是一个独立的运营工具，由管理员生成，可被任何用户用于直接增加积分，与注册流程解耦。
初始积分的可配置性: 新用户注册时赠送的积分数量应设计为后台可配置的参数，而非硬编码。这便于根据运营策略（如公测期、节假日活动）灵活调整，无需修改代码和重新部署。
原子性操作: 所有修改用户积分的操作必须是原子性的，并同步创建明细记录。在数据库层面，将使用 SELECT ... FOR UPDATE 事务锁来实现，确保数据最终一致性。
成本预估与结算: 成本预估在前端进行，以便实时反馈；后端在执行扣费时，必须基于接收到的数据独立、重新计算最终消耗积分，作为唯一的结算依据，防止客户端篡改。
初期定价模型: 为简化用户理解和快速上线，V1 版本采用 “100 字符 ≈ 1 积分” 的固定线性计费模型。更精确的、基于 Token 的计费模型将在后续版本中考虑。

已知取舍与风险 (Known Trade-offs & Risks)
风险: 积分计算或扣除逻辑的 Bug 可能导致用户资产损失，引发用户不满。所有相关代码必须有严格的测试覆盖和监控告警。
取舍: 初期积分为整数，不支持小数，以简化计算。积分明细初期只记录核心信息，暂不提供复杂的筛选和统计功能。

🚀 5. 未来范围 / V2+ (Future Scope / V2+)
此列表用于长期规划，不包含在 V1 和 V1.x 的范围之内

商业化功能 (Monetization)
[ ] 积分充值: 允许用户直接购买积分包（对接 Stripe, Lemon Squeezy, 或微信/支付宝支付）。
[ ] 订阅套餐: 提供包月/包年订阅，包含固定的积分额度，并可能享有其他高级功能。

用户留存与激励 (Engagement & Retention)
[ ] 积分有效期: 为部分赠送的积分（如活动奖励）设置过期时间，以促进消费。
[ ] 任务系统: 引入更多任务（如“完成首次 PDF 解析”），完成后给予一次性积分奖励。

体验增强 (Experience Enhancemnts)
[ ] 高级成本预估: 在预估成本时，不仅基于文本长度，还能分析内容复杂度（如代码、专业术语）、或文件类型（PDF 中的图片占比），提供更精准的消耗预估。

系统高级功能 (Advanced Features)
[ ] 用量告警: 当用户积分低于某个阈值时，通过邮件或站内信提醒。
...
[ ] AI 集成: 将获取到的字幕文本传递给我们现有的 ai_processor 进行分析和结构化，可能需要微调 Prompt 以更好地处理口语化内容。
...
字幕质量: 依赖 YouTube 自动生成的字幕质量可能参差不齐，这会直接影响最终生成的思维导图的质量。这是当前 MVP 阶段接受的技术限制。
开发/测试提示 (Dev/Test Tips)
...
🎯 1. 目标与价值 (Objective & Value)
将 ThinkSo 的内容处理能力从单体音频文件扩展到系列播客节目，允许用户通过提供播客的 RSS Feed 链接，选择任意一集节目并将其内容快速结构化为思维导图。这将精准服务于播客的重度听众、深度学习者和内容创作者，进一步巩固 ThinkTree 作为深度内容分析工具的领先地位。
...
[ ] 组件: 在输入组件中，集成清晰的用户引导文案，明确告知支持的链接类型和查找方法。具体文案如下：

标题: 输入播客 RSS Feed 链接
占位提示: 例如: https://feeds.fireside.fm/test/rss
详细说明:
✅ 我们需要：播客的 RSS Feed 公开订阅链接。
❌ 不支持：Apple Podcasts, Spotify, 小宇宙等 App 的节目分享链接。
💡 如何找到？：您通常可以在播客官网，或在 Overcast, Pocket Casts 等泛用型播客客户端中找到“复制 RSS Feed”的选项。
🔒 请注意：链接必须是公开可访问的，不支持需付费或登录的私密播客。
