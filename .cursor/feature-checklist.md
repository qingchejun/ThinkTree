...
[x] UI: 在“用量与计费”页面，增加兑换码输入框和兑换按钮
[x] 提示: 当用户积分不足时，显示友好提示，并提供一个指向“邀请好友”页面的链接
[x] 提示: 用户获得每日登录奖励后，弹出成功的提示（例如 Toast）
[x] 提示: 处理兑换码兑换后的成功或失败提示
...
Feature: Google OAuth 登录系统 (Google OAuth Login System)
Status: ✅ 已完成
Owner: @thinkso-team
Last Updated: 2025-08-02

🎯 1. 目标与价值 (Objective & Value)
集成 Google OAuth 2.0 登录功能，为用户提供一键登录体验，降低注册门槛，提升用户转化率。同时保留传统邮箱+密码登录作为备选方案，确保用户登录的多样性和安全性。

✅ 2. 验收标准 (Definition of Done)
[x] 基础功能: 首页登录浮窗（LoginModal）包含 Google 登录按钮。
[x] 核心流程: 点击"使用 Google 登录"能够成功触发 OAuth 授权流程。
[x] 核心流程: Google 授权后用户直接登录成功并跳转到工作台。
[x] 数据库支持: User 模型增加 google_id 字段，支持 Google 账户关联。
[x] 账户关联: 现有邮箱用户可以关联 Google 账户，新用户自动创建。
[x] 安全性: OAuth 2.0 标准安全流程，JWT token 集成现有认证体系。
[x] 用户体验: OAuth 回调页面提供状态反馈和自动跳转。
[x] 积分集成: Google 登录用户自动获得初始积分和每日奖励。
[x] 兼容性: 与现有传统登录系统完全兼容，不影响现有用户。
[x] 部署就绪: 所有代码完成并推送，配置说明文档完整。

📋 3. 任务分解 (Task Breakdown)
后端 / API (Backend / API)
[x] 依赖管理: 添加 authlib>=1.3.0 到 requirements.txt
[x] 数据库模型: 在 User 模型中添加 google_id 字段，设置 password_hash 为可空
[x] OAuth 配置: 创建 backend/app/core/oauth.py 配置 Google OAuth 客户端
[x] API 路由: 在 auth.py 中添加 /google 和 /google/callback 路由
[x] 用户关联逻辑: 实现新用户创建和现有用户 Google 账户关联
[x] JWT 集成: OAuth 登录成功后生成 JWT token 并重定向到前端
[x] 积分系统集成: Google 用户自动获得初始积分和每日奖励
[x] 错误处理: 完善 OAuth 流程的异常捕获和错误重定向

前端 / UI (Frontend / UI)
[x] 登录浮窗: LoginModal 组件集成 Google 登录按钮
[x] OAuth 回调页面: 创建 /auth/callback 页面处理 OAuth 回调
[x] 状态管理: 集成 AuthContext 处理 OAuth 登录状态
[x] 用户反馈: 回调页面提供处理状态、成功和错误的视觉反馈
[x] 自动跳转: 登录成功后自动跳转到工作台，失败时返回登录页
[x] 每日奖励提示: 显示每日登录奖励获得的用户通知
[x] 错误处理: 处理 OAuth 流程中的各种错误情况

💡 4. 注意事项与技术决策 (Notes & Technical Decisions)
技术决策 (Technical Decisions)
OAuth 标准: 采用 Google OAuth 2.0 标准流程，使用 authlib 库确保安全性和稳定性。
用户识别: 系统以邮箱地址为核心身份标识，Google 用户通过 google_id 字段关联。
会话管理: 使用 JWT token 管理用户会话，与现有认证体系无缝集成。
账户关联: 支持现有邮箱用户关联 Google 账户，新用户自动创建账户。
向后兼容: 完全保留传统登录系统，Google 登录作为便捷选项补充。

已知取舍与风险 (Known Trade-offs & Risks)
外部依赖: Google OAuth 服务的可用性影响登录功能，已通过传统登录备选方案缓解。
隐私考虑: 用户需要授权应用访问基本 Google 信息，符合最小权限原则。
国内访问: Google 服务在某些地区可能受限，传统登录提供替代方案。

部署配置要求 (Deployment Requirements)
环境变量: 需要配置 GOOGLE_CLIENT_ID 和 GOOGLE_CLIENT_SECRET
Google Cloud: 配置 OAuth 回调 URL 为 https://thinktree-backend.onrender.com/api/auth/google/callback
数据库迁移: 需要执行迁移添加 google_id 字段到 users 表
